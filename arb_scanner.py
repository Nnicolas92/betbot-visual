...
Obtiene los deportes disponibles dinamicamente de la API (evita 404).
...
# ── OBTENER DEPORTES DISPONIBLES DINAMICAMENTE ───────────────
def get_sports_disponibles():
    """Obtiene la lista real de deportes activos en tu cuenta."""
    ...
    try:
        ...
        # Solo deportes activos (con partidos en curso o proximos)
        activos = [s["key"] for s in data if s.get("active", False)]
        print(f"  [OddsAPI] {len(activos)} deportes activos encontrados")
        return activos
    except Exception as e:
        print(f"  [OddsAPI] Error obteniendo deportes: {e}")
        return []
...
def fetch_odds_api(sport):
    ...
    url = (f"https://api.the-odds-api.com/v4/sports/{sport}/odds/"
           f"?apiKey={ODDS_API_KEY}&regions=eu,us,uk&markets=h2h"
    ...
    try:
        ...
        if "404" not in str(e):  # silenciar 404, mostrar otros errores
...
async def scan_once():
    global _sports_cache, _sports_last_fetch
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"[{ts}] Escaneando...", end=" ", flush=True)

    eventos = []

    # Actualizar lista de deportes cada 10 minutos
    import time
    if ODDS_API_KEY and (time.time() - _sports_last_fetch > 600 or not _sports_cache):
        _sports_cache = get_sports_disponibles()
        _sports_last_fetch = time.time()

    if ODDS_API_KEY and _sports_cache:
        for sport in _sports_cache:
...
async def main():
    print("""
============================================================
  BETBOT SCANNER v3.1 - Surebets Garantizados
